...
class LocationRepository
{
    public function find(int $id): ?array
    {
        $stmt = Database::app()->prepare(
            'SELECT id, parent_id, name, kind, note FROM location WHERE id = ? LIMIT 1'
        );
        $stmt->execute([$id]);
        $row = $stmt->fetch();
        return $row ?: null;
    ...
    {
        return Database::app()
            ->query('SELECT id, parent_id, name, kind FROM location ORDER BY kind, name')
            ->fetchAll();
    }
    ...
    public function byKinds(array $kinds): array
    {
        ...
        $stmt = Database::app()->prepare(
            'SELECT l.id, l.name, l.kind, ll.code
             FROM location l
             LEFT JOIN location_label ll ON ll.location_id = l.id
    ...
    {
        $rows = Database::app()
            ->query('SELECT id, parent_id FROM location')
            ->fetchAll();
        $childrenOf = [];
        foreach ($rows as $r) {
